...
test("a model update for a current team agent missing from the registry still lands", async () => {
  ...
  // through editor.update rather than skip a missing entry. The upsert is
  // allowed only because the id is a current team winner (teamAgents).
  const agents = agentHarness([agentInfo("alpha", "upstream")])
  const ctx = context({ agent: agents.domain })
  const discovered = await discoverFor(ctx)
  const models = [
...
test("a model update for an agent missing from the registry and outside the team creates nothing", async () => {
  // The same retained model record must not resurrect a disabled team role:
  // an absent id outside the current team winners keeps the old skip-if-absent
  // behaviour, so no phantom role with default permissions is created.
  const agents = agentHarness([agentInfo("alpha", "upstream")])
  const ctx = context({ agent: agents.domain })
  const discovered = await discoverFor(ctx)
  const models = [
  ...
  expect(applied.registrations).toHaveLength(1)
  expect(agents.state.get("alpha")?.model).toMatchObject({ providerID: "acme", id: "nova-2" })
  expect(agents.state.has("ghost")).toBe(false)
  expect(agents.state.get("ghost")).toBeUndefined()
})
