...
- **Plugin unload does not revoke applied transforms.** `applied` registrations in `packages/plus/src/instructions/apply.ts` are created on detached scopes (`Scope.make()` driven by `Effect.runPromise`), and `packages/plus/src/index.ts` registers no finalizer on the owning plugin scope. Core closes only the plugin activation scope (`packages/core/src/plugin.ts`), so if Plus were unloaded or hot-replaced mid-session its transforms would stay installed and a replacement instance could observe the previous instance's output as upstream. This leak is reachable: `packages/core/src/config/plugin/source.ts` parses plugin removals such as `-opencode.plus`, `packages/core/src/plugin/supervisor.ts` applies those removals to built-in plugin definitions and orders external packages before `post` built-ins, and `packages/core/src/plugin.ts` closes and reactivates the entire changed suffix of the plugin list — which includes Plus — whenever an earlier plugin changes. Previously applied hooks and transforms survive that unload or reactivation. A concrete consequence: an old native-tool deletion hook survives reactivation, and a later reset disposes only the new instance's registrations, leaving the tool disabled with nothing able to re-enable it. Closing this requires registering teardown on the owning plugin scope; documentation alone is not sufficient and this remains an open defect.
