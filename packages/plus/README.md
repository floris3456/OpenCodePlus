...
- **Inherited review provenance binds to text origin.** When a shared Defaults record carries both customized `text` and a `needs review` flag raised by an upstream change, an agent row that inherits that record can be toggled to override state independently. `override()` in `packages/plus/src/instructions/model.ts` now binds review provenance (`basedOn` and `reviewed`) to the record supplying the text (the shared record when the agent only overrides state), so a state-only toggle no longer clears an unacknowledged review while the stale shared text remains applied. Note that a separate fix for acknowledging reviews directly from an agent row is landing in parallel, so this defines the boundary between text provenance and state overrides rather than guaranteeing the full review workflow.
