...
Up/down move, left collapse/parent, right expand, Enter edit text (or diff on yellow review rows, rule editor on permission rows), Space toggle include/exclude (on a Models row: activate exclusively at that level; on a rule row: toggle the rule), `p` pin (Code Mode tool rows), `a` add (`a` on a Models group adds a catalog candidate; `a` on a team row adds an agent to that team; `a` on an eligible tool row offers Section or Permission rule scoped to that row, prompting otherwise), `d` delete (`d` on a model row deletes the candidate at that level; `d` on a rule row deletes only user-created rules), `r` reset override (`r` on a model row clears only that level's active flag), `s` split, `/` filter, `?` help, esc back. Inside the diff: `k` keep mine, `t` take new, `e` edit.
