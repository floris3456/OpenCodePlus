...
`Defaults` holds `Agents` (in the same origin subgroups), `Teams` (built-in shipped teams with working toggles, whose team rows carry `add: "agent"` and whose member rows expand to full agent subtrees; `add` creates at project or global scope), followed by the shared inventories: `Models` `[a]`, `Tools`, `Base` `[a]`, `Skills`, `System` `[a]`, `MCP` `[a: add MCP server]`. `a` on an Agents group or User subgroup adds an agent at that level; `a` on a team row adds an agent to that team. `d` delete is offered only on rows whose `actions.remove === true`: project/global agents (`agent.delete`), shared MCP servers (`mcp.remove`), project-owned skills (`skill.delete`), user base templates (`base.delete`), project instruction files (`instruction.delete`), local model candidates (`removeModelRecord`), and user-created permission rules (`rule.remove`); rows without remove actions do not bind `d` or offer it in footer hints.
...
Up/down move, left collapse/parent, right expand, Enter edit text (or diff on yellow review rows, rule editor on permission rows), Space toggle include/exclude (on a Models row: activate exclusively at that level; on a rule row: toggle the rule), `p` pin (Code Mode tool rows), `a` add (`a` on an Agents group or User subgroup adds an agent; `a` on a Models group adds a catalog candidate; `a` on a Teams group creates a team; `a` on a team row adds an agent to that team; `a` on an eligible tool row offers Section or Permission rule scoped to that row, prompting otherwise), `d` delete (offered only on rows with `actions.remove === true`: deletes project/global agents, shared MCP servers, project skills, user base templates, project instructions, local model candidates, and user-created permission rules; prompts for confirmation), `r` reset override (`r` on a model row clears only that level's active flag), `s` split, `/` filter, `?` help, esc back. Inside the diff: `k` keep mine, `t` take new, `e` edit.
