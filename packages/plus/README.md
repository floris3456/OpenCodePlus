...
- **`write`/`edit` share one enforcement action; `patch` adds per-operation actions.** Core asserts `action: "edit"` for write/edit (core/src/tool/plugin/edit.ts, write.ts) and, for patch, per-operation `patch.add`/`patch.update`/`patch.delete` (bare paths for that type) before its unchanged `edit` assert (core/src/tool/plugin/patch.ts); Plus prefers the per-rule `permAction` carried on perm items and falls back to that map. A wildcard-action `ask` can see one extra prompt per patch.
