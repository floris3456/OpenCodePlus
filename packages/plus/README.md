...
- `patches/@opentui%2Fcore@0.5.10.patch` — stdin parser holds lone ESC for 50 ms, flushes ESC-ESC immediately, clears the ESC-recovery flag once escape dispatches, and gates kitty parsing off until the terminal confirms it via `CSI ? <flags> u`. Upstream: `anomalyco/opencode#37692` (`setupTerminal()` never calls `enableKittyKeyboard()`) and `anomalyco/opentui#818` / PR `#819` (CSI timeout flush drops split escape sequences). (`packages/tui/test/stdin-esc.test.ts`)
