...
- `packages/core/src/tool/plugin/patch.ts` — edit permission resources now include `<type>:<resource>` alongside the bare path so a rule can distinguish add from delete; the hunk type never reached the permission layer, so no plugin could recover it. (`packages/core/test/tool-patch.test.ts`)
