...
All timestamps are recorded in ISO 8601 UTC directly from the committed per-cycle driver logs (`soak-2026-09-22-fixed/cycles/cycle-NN.json`) and summary receipts (`soak-2026-09-22-fixed/summary-1790086656.json` and `soak-2026-09-22-fixed/summary-1790087052.json`). The **Wall Time (Start→End)** column reports true elapsed cycle wall-clock time computed from each row's own `started` and `ended` timestamps. For paired cycles, the original report's paired-cycle `Elapsed` figures (`elapsedSec` in the original driver summary) came from a runner that summed driver-call durations, which is significantly lower for the second child (e.g. 3.2s for Cycle 9 vs. 23.1s wall time, 3.5s for Cycle 11 vs. 29.9s wall time, 4.2s for Cycle 13 vs. 34.1s wall time, 5.5s for Cycle 15 vs. 27.0s wall time, and 13.9s for Cycle 26 vs. 49.2s wall time) because `elapsedSec` measured only active driver subprocess execution time and excluded the queue time spent in wait while its simultaneous partner integrates. The runner now records `wallElapsedSec` (delegate start through the end of integration, the tail call) and keeps the sum separately as `driverActiveSec`; the original raw captures are unchanged. Both figures are preserved below. Outcomes are strictly classified as `landed` or `failed`. Every cycle's merge was verified against its own merge entry (see Section 2.1 for receipt retention details across cycles 1–15 vs. 16–30).
...
- **Durable Receipts**: [`soak-2026-09-22-fixed/supplementary/receipts/cycle-31.json`](soak-2026-09-22-fixed/supplementary/receipts/cycle-31.json) through `cycle-45.json` — **15 cycles**, each exported with its own `mergeReceipt`, its own `settled` entry, `integrate` output, and `sourceHead`. Receipts for cycles 39–45 carry a per-cycle `model` field; receipts 31–38 were back-filled after their cycles ran and do not contain a `model` field, relying on the shared pre-delegation `/api/agent` proof ([`model-proof.txt`](soak-2026-09-22-fixed/supplementary/captures/model-proof.txt)).
...
In the original report, paired-cycle `Elapsed` figures reflected a runner that summed individual driver-call durations (`elapsedSec`). In the updated driver used for `resoak4`, the runner records true wall-clock time as `wallElapsedSec` (measured from delegate start through the end of integration, the tail call — dispatch to the completion of that child's own integrate, not merely until the wait returned) and keeps the subprocess execution sum separately as `driverActiveSec`. The original raw captures remain unchanged.
...
**State the limit honestly:** This is a **field-level guarantee for `worktree` only**, not a general lost-update fix; other fields written from a stale copy can still be overwritten. Do **not** write that `"dirty"` is harmless because it is not `"present"` — writing `dirty` over `removed` lies about the directory just as much. The boundary latch strictly preserves `"removed"` against `"present"`, `"dirty"`, and omitted fields alike.
