...
2. **Risk Direction**: Unit test coverage (`test/release/canonicalize.test.ts:1404-1406`) verifies that in real Bun 1.4.2 builds, builtin bytecode blobs and module subranges are mutually disjoint allocations. Furthermore, neither builtin bytecode blobs nor module subranges are eligible for chunk-token normalization. Any differing byte in either region is treated as non-derivable and rejected with `residual-difference`.
