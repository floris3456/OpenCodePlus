...
3. **The gate asserts `resources: ["*"]` for plugin tools**, so a deny narrower than `*` on a plugin tool refuses every call to it. That is the documented consequence of D2, not an accident.
