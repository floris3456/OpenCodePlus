...
5. **`.opencode` directories** - agents, commands, tools
...
:::note
The `.opencode` and `~/.config/opencode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/` (global only), `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
environment variable. This directory overrides the global config directory
(`~/.config/opencode`) and will be searched for agents, commands,
modes, and plugins following the same structure.
...
Executable plugins are loaded only from trusted host-level sources:

- **Global configuration**: `~/.config/opencode/opencode.json` (or `~/.config/opencode/plugins/`)
- **Explicit custom config**: A config file specified via the `OPENCODE_CONFIG` environment variable
- **Inline config**: Host-injected configuration via the `OPENCODE_CONFIG_CONTENT` environment variable
- **Remote configuration**: Organizational defaults via `.well-known/opencode`
...
Project-local and repository-local declarations do not load executable plugins. Defining `plugin` in a project-level `opencode.json` or placing files in a project `.opencode/plugins/` directory is refused and will not execute; configuration documents discovered through the upward project directory walk cannot select executable modules.
